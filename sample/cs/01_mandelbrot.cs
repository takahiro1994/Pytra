...
public static class Program
{
    ...
    public static List<byte> render_mandelbrot(long width, long height, long max_iter, double x_min, double x_max, double y_min, double y_max)
    {
        ...
        for (y = 0; y < height; y += 1) {
            double py = y_min + (y_max - y_min) * (System.Convert.ToDouble(y) / System.Convert.ToDouble((height - 1)));
            
            long x = 0;
            for (x = 0; x < width; x += 1) {
                double px = x_min + (x_max - x_min) * (System.Convert.ToDouble(x) / System.Convert.ToDouble((width - 1)));
                ...
                if ((it) >= (max_iter)) {
                    ...
                    double t = System.Convert.ToDouble(it) / System.Convert.ToDouble(max_iter);
