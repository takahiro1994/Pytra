...
def run_15_wave_interference_loop(): Unit = {
    ...
    while (t < frames_n) {
        ...
        while (y < h) {
            ...
            while (x < w) {
                var dx: Long = x - 160L
                var dy: Long = y - 120L
                var v: Double = ((math_native.sin((((__pytra_float(x) + __pytra_float(t) * 1.5)) * 0.045)) + math_native.sin((((__pytra_float(y) - __pytra_float(t) * 1.2)) * 0.04)) + math_native.sin((((__pytra_float(x + y)) * 0.02) + phase))) + math_native.sin((math_native.sqrt((dx * dx + dy * dy)) * 0.08 - phase * 1.3)))
    ...
    save_gif(out_path, w, h, frames, grayscale_palette(), 4L, 0L)
...
def main(args: Array[String]): Unit = {
    val _ = run_15_wave_interference_loop()
}