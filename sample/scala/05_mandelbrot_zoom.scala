...
def render_frame(width: Long, height: Long, center_x: Double, center_y: Double, scale: Double, max_iter: Long): mutable.ArrayBuffer[Long] = {
    ...
    while (y < height) {
        ...
        while (x < width) {
            ...
            __pytra_set_index(frame, row_base + x, __pytra_int(255.0 * __pytra_float(i) / __pytra_float(max_iter)))
            x += 1L
        }
        y += 1L
    }
...
def run_05_mandelbrot_zoom(): Unit = {
    ...
    save_gif(out_path, width, height, frames, grayscale_palette(), 5L, 0L)
