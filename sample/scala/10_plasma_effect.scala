    ...
    while (t < frames_n) {
        ...
        while (y < h) {
            ...
            while (x < w) {
                var dx: Long = x - 160L
                var dy: Long = y - 120L
                var v: Double = ((math_native.sin((((__pytra_float(x) + __pytra_float(t) * 2.0)) * 0.045)) + math_native.sin((((__pytra_float(y) - __pytra_float(t) * 1.2)) * 0.05)) + math_native.sin((((__pytra_float(x + y) + __pytra_float(t) * 1.7)) * 0.03))) + math_native.sin((math_native.sqrt((dx * dx + dy * dy)) * 0.07 - __pytra_float(t) * 0.18)))
