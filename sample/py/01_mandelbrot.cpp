...
::std::tuple<int64, int64, int64> color_map(int64 iter_count, int64 max_iter) {
    ...
    float64 t = float64(iter_count) / float64(max_iter);
...
bytearray render_mandelbrot(int64 width, int64 height, int64 max_iter, float64 x_min, float64 x_max, float64 y_min, float64 y_max) {
    ...
    for (int64 y = 0; y < height; ++y) {
        float64 py = y_min + (y_max - y_min) * (float64(y) / __hoisted_cast_1);
        
        for (int64 x = 0; x < width; ++x) {
            float64 px = x_min + (x_max - x_min) * (float64(x) / __hoisted_cast_2);
            ...
            if (it >= max_iter) {
                ...
                float64 t = float64(it) / __hoisted_cast_3;
                r = int64(255.0 * t * t);
                g = int64(255.0 * t);
                b = int64(255.0 * (1.0 - t));
            }
