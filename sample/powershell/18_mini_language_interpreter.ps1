...
function tokenize {
    ...
    for ($_i = $null; $_i -lt $null; $_i += 1) {
        ...
        while (($i -lt $n)) {
            ...
            if ((& (Get-Command ("{0}_{1}" -f $ch["__type__"], "isdigit")) $ch)) {
                $start = $i
            ...
            if (((& (Get-Command ("{0}_{1}" -f $ch["__type__"], "isalpha")) $ch) -or ($ch -eq "_"))) {
                $start = $i
...
function Parser {
    ...
    $self["tokens"] = $tokens
    $self["pos"] = 0
    $self["expr_nodes"] = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "new_expr_nodes")) $self)
}
...
function Parser_peek_kind {
    param($self)
    return (& (Get-Command ("{0}_{1}" -f $self["__type__"], "current_token")) $self).kind
}
function Parser_match {
    param($self, $kind)
    if (((& (Get-Command ("{0}_{1}" -f $self["__type__"], "peek_kind")) $self) -eq $kind)) {
        $self["pos"] += 1
        return $true
    }
    return $false
...
function Parser_expect {
    param($self, $kind)
    $token = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "current_token")) $self)
...
function Parser_skip_newlines {
    param($self)
    while ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "NEWLINE")) {
        # pass
    }
}
...
function Parser_parse_program {
    param($self)
    $stmts = @()
    (& (Get-Command ("{0}_{1}" -f $self["__type__"], "skip_newlines")) $self)
    while (((& (Get-Command ("{0}_{1}" -f $self["__type__"], "peek_kind")) $self) -ne "EOF")) {
        $stmt = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_stmt")) $self)
        $stmts += @($stmt)
        (& (Get-Command ("{0}_{1}" -f $self["__type__"], "skip_newlines")) $self)
    }
    return $stmts
...
function Parser_parse_stmt {
    param($self)
    if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "LET")) {
        $let_name = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "expect")) $self "IDENT").text
        (& (Get-Command ("{0}_{1}" -f $self["__type__"], "expect")) $self "EQUAL")
        $let_expr_index = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_expr")) $self)
        return & { $__obj = @{}; (StmtNode $__obj "let" $let_name $let_expr_index 1); $__obj }
    }
    if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "PRINT")) {
        $print_expr_index = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_expr")) $self)
        return & { $__obj = @{}; (StmtNode $__obj "print" "" $print_expr_index 3); $__obj }
    }
    $assign_name = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "expect")) $self "IDENT").text
    (& (Get-Command ("{0}_{1}" -f $self["__type__"], "expect")) $self "EQUAL")
    $assign_expr_index = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_expr")) $self)
    return & { $__obj = @{}; (StmtNode $__obj "assign" $assign_name $assign_expr_index 2); $__obj }
}
function Parser_parse_expr {
    param($self)
    return (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_add")) $self)
}
function Parser_parse_add {
    param($self)
    $left = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_mul")) $self)
    while ($true) {
        if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "PLUS")) {
            $right = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_mul")) $self)
            $left = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "bin" 0 "" "+" $left $right 3 1); $__obj })
            continue
        }
        if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "MINUS")) {
            $right = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_mul")) $self)
            $left = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "bin" 0 "" "-" $left $right 3 2); $__obj })
            continue
        }
        break
    }
...
function Parser_parse_mul {
    param($self)
    $left = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_unary")) $self)
    while ($true) {
        if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "STAR")) {
            $right = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_unary")) $self)
            $left = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "bin" 0 "" "*" $left $right 3 3); $__obj })
            continue
        }
        if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "SLASH")) {
            $right = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_unary")) $self)
            $left = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "bin" 0 "" "/" $left $right 3 4); $__obj })
            continue
        }
        break
    }
...
function Parser_parse_unary {
    param($self)
    if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "MINUS")) {
        $child = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_unary")) $self)
        return (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "neg" 0 "" "" $child (-1) 4 0); $__obj })
    }
    return (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_primary")) $self)
}
function Parser_parse_primary {
    param($self)
    if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "NUMBER")) {
        $token_num = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "previous_token")) $self)
        return (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "lit" $token_num.number_value "" "" (-1) (-1) 1 0); $__obj })
    }
    if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "IDENT")) {
        $token_ident = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "previous_token")) $self)
        return (& (Get-Command ("{0}_{1}" -f $self["__type__"], "add_expr")) $self & { $__obj = @{}; (ExprNode $__obj "var" 0 $token_ident.text "" (-1) (-1) 2 0); $__obj })
    }
    if ((& (Get-Command ("{0}_{1}" -f $self["__type__"], "match")) $self "LPAREN")) {
        $expr_index = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "parse_expr")) $self)
        (& (Get-Command ("{0}_{1}" -f $self["__type__"], "expect")) $self "RPAREN")
        return $expr_index
    }
    $t = (& (Get-Command ("{0}_{1}" -f $self["__type__"], "current_token")) $self)
...
function run_demo {
    ...
    $stmts = (& (Get-Command ("{0}_{1}" -f $parser["__type__"], "parse_program")) $parser)
