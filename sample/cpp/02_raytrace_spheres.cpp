        ...
        for (int64 x = 0; x < width; ++x) {
            ...
            for (int64 ay = 0; ay < aa; ++ay) {
                for (int64 ax = 0; ax < aa; ++ax) {
                    float64 fy = (float64(y) + (float64(ay) + 0.5) / __hoisted_cast_1) / __hoisted_cast_2;
                    float64 fx = (float64(x) + (float64(ax) + 0.5) / __hoisted_cast_1) / __hoisted_cast_3;
                    float64 sy = 1.0 - 2.0 * fy;
                    float64 sx = (2.0 * fx - 1.0) * (float64(width) / __hoisted_cast_4);
            ...
            int64 samples = aa * aa;
            pixels.append(uint8(ar / samples));
            pixels.append(uint8(ag / samples));
            pixels.append(uint8(ab / samples));
        }
    }
    return pixels;
}
