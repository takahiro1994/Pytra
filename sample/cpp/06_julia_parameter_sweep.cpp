...
bytes render_frame(int64 width, int64 height, float64 cr, float64 ci, int64 max_iter, int64 phase) {
    ...
    for (int64 y = 0; y < height; ++y) {
        ...
        float64 zy0 = -(1.2) + 2.4 * (float64(y) / __hoisted_cast_1);
        for (int64 x = 0; x < width; ++x) {
            float64 zx = -(1.8) + 3.6 * (float64(x) / __hoisted_cast_2);
            float64 zy = zy0;
            int64 i = 0;
            while (i < max_iter) {
                float64 zx2 = zx * zx;
...
void run_06_julia_parameter_sweep() {
    ...
    for (int64 i = 0; i < frames_n; ++i) {
        float64 t = float64((i + start_offset) % frames_n) / __hoisted_cast_3;
