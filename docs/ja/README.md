...
> **2026-04-04** — emitter lint に Python メソッド名ハードコード検出追加（23 パターン）。parity check の全件 SKIP → FAIL 修正。pytra-cli2.py デフォルト出力先を work/tmp/ に変更。lint 常時 10 カテゴリ化。callable_optional_none fixture 追加。emitter guide §12.6 callable 型写像・§14.1 lint ドキュメント追加。mapping.json 完全修飾キー計画起票。Zig fixture 146/146 PASS。Julia fixture 145/146。

> **2026-04-03** — emitter 呼び出し構造統一（共通ランナー 17 言語）。`resolved_type: "object"` 全面禁止。IsInstance `PYTRA_TID_*` 廃止。`--opt-level` 改名。opaque 型 spec 確定。`builtin_name` 廃止。全件 SKIP → FAIL。`tools/unregistered/` 削除。

> **2026-04-02** — C++ monostate → `optional<variant>`。bounds check EAST optimizer 移管（mandelbrot 12.8s → 0.82s）。png.py/gif.py extend 最適化。emitter guide §12.4-12.6 新設。fixture リネーム。
