...
3. [ ] [ID: P1-CS-EMITTER-S3] fixture 全件の C# emit 成功を確認する（`runtime_parity_check_fast.py --targets cs` の既存経路で確認する。2026-03-30: `--category core` は 22/22 pass、`--category collections` は 20/20 pass、`--category control` は 16/16 pass、`--category imports` は 7/7 pass、`--category oop` は 18/18 pass。未解消は `--category signature` 5/13 pass、`--category strings` 6/12 pass、`--category typing` 8/23 pass）
