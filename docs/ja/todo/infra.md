...
1. [x] [ID: P5-PARITY-EMIT-S1] 各言語の emitter.py で emit 関数名を `emit_<lang>_module` に統一する（ruby: `emit_ruby_module = transpile_to_ruby` エイリアス追加）
2. [x] [ID: P5-PARITY-EMIT-S2] `_transpile_in_memory` の emit ループを動的 import + 共通ループに置換する（importlib + `_EMIT_LANG_CONFIG` テーブル。cpp/java/js は特殊処理で残す）
3. [x] [ID: P5-PARITY-EMIT-S3] parity check 内の `_copy_<lang>_runtime` 重複実装を削除し、cli.py の post_emit に委譲する（cs/scala/kotlin/zig を削除。`_call_runtime_copier` が cli.py から動的取得し、なければローカルフォールバック）
4. [x] [ID: P5-PARITY-EMIT-S4] 全言語の parity check（fixture 代表ケース）に回帰がないことを確認する（cpp/scala/cs/kotlin/ruby/go 全て `(True, '')` 確認済み）
