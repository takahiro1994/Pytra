...
1. [ ] [ID: P1-JULIA-EMITTER-S1] `src/toolchain2/emit/julia/` に Julia emitter を新規実装する — CommonRenderer + override 構成。旧 `src/toolchain/emit/julia/` と TS emitter を参考にする
   ...
   - 2026-04-04: 上記 3 件を `load_east3_document(..., target_lang='julia')` で個別確認し、いずれも self-hosted parser の `user_syntax_error`（`Pytra parser does not support this expression syntax yet: .`）で停止することを確認。Julia emitter 側の未対応ではない
...
5. [x] [ID: P1-JULIA-EMITTER-S5] fixture の Julia run parity を通す（`julia`）
   ...
   - 2026-04-04: emitter guide の再点検に合わせ、Julia subset の builtin/method lowering を `mapping.json` 正本ベースへ寄せた。`len` / `dict.clear/pop/setdefault` / `list.extend/index/reverse/sort` / `py_range` / string method 群の mapping と marker 展開を整理し、`runtime_parity_check_fast.py --case-root fixture --targets julia` は 146/146 PASS を維持
