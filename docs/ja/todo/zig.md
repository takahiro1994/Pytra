...
4. [x] [ID: P0-ZIG-CREXC-S4] Rust / Zig の `exception_types` / `try_raise` / `exception_bare_reraise` parity を維持したまま custom lowering を縮小する
   ...
   完了メモ: S4 の目的は「backend emitter に残っていた full custom lowering を CommonRenderer / profile / helper 群へ押し戻すこと」であり、この目的は達成済み。現在 Zig に残るのは slot 名、exception match 条件、labeled block / `break :label`、fallback `with` protocol call、bound exception binding といった target syntax adapter だけで、full custom lowering ではない。Rust 側の大きい custom orchestration は吸収済み。
...
1. [x] [ID: P0-ZIG-NEWFIX-S1] 上記 fixture/stdlib の parity を確認する（対象 fixture のみ実行）
   完了メモ: `runtime_parity_check_fast.py` で Zig を対象に `bytes_copy_semantics`, `negative_index_comprehensive`, `negative_index_out_of_range`, `callable_optional_none`, `str_find_index`, `eo_extern_opaque_basic`(emit-only), `math_extended`, `os_glob_extended` を確認し、対象 8 件すべて PASS を確認した。
