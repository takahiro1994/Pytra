...
4. [ ] [ID: P0-ZIG-CREXC-S4] Rust / Zig の `exception_types` / `try_raise` / `exception_bare_reraise` parity を維持したまま custom lowering を縮小する
   ...
   残件: Zig 側は manual exception slot を Zig 構文へ写す最終 adapter として残っている hook 群の境界整理と、dispatch state / handler prelude-teardown の最終抽象化。Rust 側は `emit_custom_with_stmt()` 自体をさらに CommonRenderer 正本へ吸収するかどうかの整理。
