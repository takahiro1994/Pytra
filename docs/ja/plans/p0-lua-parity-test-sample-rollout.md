<a href="../../en/plans/p0-lua-parity-test-sample-rollout.md">
...
- sample parity（`sample/py` 18件）が `--targets lua` で全 pass する。
...
- [x] [ID: P0-LUA-PARITY-ALL-01-S2-02] `runtime_parity_check --case-root sample --targets lua` を実行し、不一致を修正して全 pass にする。
...
- 2026-03-01: `python3 tools/check/runtime_parity_check.py --case-root sample --targets lua --ignore-unstable-stdout` は未達（pass=3, fail=15）。
...
- 2026-03-01: `python3 tools/check/runtime_parity_check.py --case-root sample --targets lua --ignore-unstable-stdout` は pass（18/18, fail=0）。
