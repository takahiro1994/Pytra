...
- 2026-04-04: [ID: P1-JULIA-EMITTER-S1] 上記 3 件を個別に `load_east3_document(..., target_lang='julia')` で確認し、全て self-hosted parser の `user_syntax_error`（`Pytra parser does not support this expression syntax yet: .`）で停止することを確認した。残件は Julia backend ではなく frontend parser 側の課題である。
