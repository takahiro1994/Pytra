...
- 2026-03-31: sample parity を継続し、`PYTHONPATH=src:tools/check python3 tools/check/runtime_parity_check_fast.py --case-root sample --targets cs` は 16/18 pass まで改善した。`01`-`16` のうち `17_monte_carlo_pi` と `18_mini_language_interpreter` のみ未解消で、残因は `pytra.std.pathlib` の `typing.cast` / imported `Path` ctor 周辺の C# emit に絞られている。
