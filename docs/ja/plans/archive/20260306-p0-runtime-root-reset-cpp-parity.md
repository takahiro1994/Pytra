<a href="../../../en/plans/archive/20260306-p0-runtime-root-reset-cpp-parity.md">
...
- `tools/gen/gen_runtime_from_manifest.py` / `tools/runtime_generation_manifest.json`（C++向け）
- parity 実行導線（`tools/check/runtime_parity_check.py`）
...
- 2026-03-05: [ID: `P0-RUNTIME-ROOT-RESET-CPP-01-S1-03`] `tools/check/check_runtime2_references_absent.py` と unit test（`test_check_runtime2_references_absent.py`）を追加し、`tools/run/run_local_ci.py` に組み込んだ。`src/runtime2` 参照の再流入を CI で fail-fast 化。
...
- 2026-03-05: [ID: `P0-RUNTIME-ROOT-RESET-CPP-01-S2-02`] runtime manifest の C++ 出力先を `src/runtime/cpp/gen/...` に更新し、`tools/gen/gen_runtime_from_manifest.py --targets cpp --items utils/png,utils/gif` で再生成して `pytra-gen` 経路の再作成を防止。
- 2026-03-05: [ID: `P0-RUNTIME-ROOT-RESET-CPP-01-S2-03`] `tools/gen/gen_makefile_from_manifest.py` / `tools/build_multi_cpp.py` / `tools/verify_sample_outputs.py` / `tools/check/verify_image_runtime_parity.py` を `runtime/cpp/{core,gen}` 前提へ更新。さらに `src/runtime/cpp/{core,gen}` 内 include を新パスへ正規化し、`python3 src/pytra-cli.py sample/py/01_mandelbrot.py --target cpp --build --output-dir out/p0_s203_build` で `core+gen` のみを用いたビルド通過を確認。
...
- 2026-03-05: [ID: `P0-RUNTIME-ROOT-RESET-CPP-01-S4-01`] `tools/check/runtime_parity_check.py --targets cpp --case-root fixture --summary-json work/logs/p0_runtime_root_reset_cpp_fixture_20260305_s401.json` を通過（`math_extended/pathlib_extended/inheritance_virtual_dispatch_multilang` = 3/3）。
...
- 2026-03-05: [ID: `P0-RUNTIME-ROOT-RESET-CPP-01-S3-02`] `tools/check/check_runtime_cpp_layout.py` と `tools/check/check_runtime_std_sot_guard.py` を `runtime/cpp/{core,gen}` 前提へ更新し、旧 `pytra-core/pytra-gen/pytra` forwarder 仮定を撤去した。`check_runtime_cpp_layout.py` / `check_runtime_std_sot_guard.py` / `check_runtime_core_gen_markers.py` を通過。
