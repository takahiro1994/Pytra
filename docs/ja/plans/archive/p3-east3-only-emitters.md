<a href="../../../en/plans/archive/p3-east3-only-emitters.md">
...
- tests/docs: `tools/unittest/test_py2*_smoke.py`, `tools/check/check_py2*_transpile.py`, `docs/ja/plans/plan-east123-migration.md` ほか関連仕様
...
- 2026-02-26: `S1-01` として 8本 CLI の `--east-stage 2` を警告から `parser.error` へ統一。`tools/unittest/test_py2{rs,cs,js,ts,go,java,swift,kotlin}_smoke.py` の stage2 警告依存を「非0終了 + エラーメッセージ期待」へ更新し、8本を実行して全通過を確認。
...
- 2026-02-26: `S2-05` として JS/TS smoke と `check_py2{js,ts}_transpile.py` を全通し。付随して `tools/check/check_noncpp_east3_contract.py` の静的契約を EAST3-only（stage2 警告期待→stage2 非対応期待、compat import 禁止）へ更新し、`test_east3_cpp_bridge.py` の `py_to<int64>/py_to<bool>` 期待値を現実装へ同期して east3-contract 前提の失敗を解消。
...
- 2026-02-26: `S5-01` として 8本 CLI から `normalize_east3_to_legacy` import/call を撤去し、`load_east` を EAST3 ドキュメント返却へ統一。対応 smoke の `east_stage` 期待を `2 -> 3` へ更新し、8本 smoke 全通過を確認。`tools/check/check_noncpp_east3_contract.py` へ `normalize_east3_to_legacy` 禁止と新 smoke 名 (`returns_east3_shape`) を反映。
...
- 2026-02-26: `S6-02` として `docs/en/plans/plan-east123-migration.md` を `docs-ja` と同内容へ同期し、`EAST3 only` 契約と注記を日英系ドキュメントで一致させた。
...
- 2026-02-26: `S7-02` として `runtime_parity_check --case-root sample --targets rs,cs,js,ts,go,java,swift,kotlin --ignore-unstable-stdout` を実行。`SUMMARY cases=18 pass=18 fail=0`・`ok: 144` を確認し、非C++ 8ターゲットの `EAST3 only` 整合確認を完了した。
...
- [x] [ID: P3-EAST3-ONLY-01-S7-02] `runtime_parity_check --case-root sample --targets rs,cs,js,ts,go,java,swift,kotlin --ignore-unstable-stdout` を実行し、整合を最終確認する。
