...
- `tools/check/check_py2lua_transpile.py` の `DEFAULT_EXPECTED_FAILS` には `sample/py` の残り14件（`01,05..16,18`）が含まれ、検証時にスキップされている。
...
- `tools/check/check_py2lua_transpile.py` の expected-fail 縮退
- `tools/gen/regenerate_samples.py --langs lua` の全件再生成
- `sample/lua/*.lua`（18件）
- `tools/unittest/test_py2lua_smoke.py` と parity 導線
...
- [x] [ID: P1-LUA-SAMPLE-FULL-01-S2-02] `tools/check/check_py2lua_transpile.py` の `DEFAULT_EXPECTED_FAILS` から sample 対象を段階削除し、スキップ依存を解消する。
