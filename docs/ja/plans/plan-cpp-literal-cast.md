...
- `CommonRenderer` に `literal_nowrap_ranges` の読み込みと整数 literal 判定を追加し、profile 駆動で bare literal / typed wrap を切り替えるようにした
- C++ profile には `int` / `int32` / `int64` の safe nowrap range を設定し、`int8` / `int16` / `uint*` は従来どおり wrap を維持した
- C++ emitter の `_emit_constant` は整数型を CommonRenderer へ委譲し、`min` / `max` だけは template deduction を壊さないよう引数 literal を target type で強制ラップする補助経路を残した
- parity の確認中に `ClosureDef` constructor で `super().__init__(...)` が init-list 化されても本文に残る既存不具合を発見したため、constructor init-list を使う場合は先頭の `super().__init__` 文を body から落とす回帰修正も併せて実施した
