<a href="../../en/plans/p0-scala-sample01-quality-uplift.md">
...
- 2026-03-02: [ID: P0-SCALA-SAMPLE01-QUALITY-01-S3-02] `sample/py/*.py -> sample/scala/*.scala` を全件再生成し、`test_py2scala_smoke`（17件）と `runtime_parity_check --targets scala`（18/18 pass）を確認した。`tools/gen/regenerate_samples.py --langs scala` は未対応（`unknown language(s): scala`）のため別 P0 で継続管理。
