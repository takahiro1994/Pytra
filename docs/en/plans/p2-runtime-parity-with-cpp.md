...
- 2026-03-05: [ID: `P2-RUNTIME-PARITY-CPP-02-S2-01`] Added `tools/check/check_runtime_pytra_gen_naming.py`, enabling static detection of naming and placement violations under `pytra-gen` for `std|utils` straight-through naming. Existing debt is explicitly tracked in `tools/runtime_pytra_gen_naming_allowlist.txt`, 11 items, and both `tools/unittest/tooling/test_check_runtime_pytra_gen_naming.py` and the main check passed.
- 2026-03-05: [ID: `P2-RUNTIME-PARITY-CPP-02-S2-02`] Added `tools/check/check_runtime_core_gen_markers.py`, making marker requirements, `source/generated-by`, under `pytra-gen` and forbidding generated-marker mixing into `pytra-core`, a static audit across all languages. Integrated both `check_runtime_core_gen_markers.py` and `check_runtime_pytra_gen_naming.py` into `tools/run/run_local_ci.py`, and confirmed passing for `test_check_runtime_core_gen_markers.py`, `test_check_runtime_pytra_gen_naming.py`, and both main checks.
