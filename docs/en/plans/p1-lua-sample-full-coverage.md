<a href="../../ja/plans/p1-lua-sample-full-coverage.md">
...
- `DEFAULT_EXPECTED_FAILS` in `tools/check/check_py2lua_transpile.py` contains the remaining 14 `sample/py` cases (`01,05..16,18`), and they are skipped during validation.
...
- Reduce expected-fail entries in `tools/check/check_py2lua_transpile.py`
- Regenerate all cases via `tools/gen/regenerate_samples.py --langs lua`
- `sample/lua/*.lua` (18 files)
- `tools/unittest/test_py2lua_smoke.py` and parity path
...
- [x] [ID: P1-LUA-SAMPLE-FULL-01-S2-02] Remove sample targets from `DEFAULT_EXPECTED_FAILS` in `tools/check/check_py2lua_transpile.py` step-by-step and eliminate skip dependency.
