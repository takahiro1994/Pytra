<a href="../../ja/plans/p2-ruby-backend-rollout.md">
...
- `tools/check/check_py2rb_transpile.py` / `tools/unittest/test_py2rb_smoke.py` / parity path
...
- `tools/check/check_py2rb_transpile.py` and smoke/parity regression paths are provided.
...
- 2026-02-27: [ID: `P2-RUBY-BACKEND-01-S3-01`] Added `tools/check/check_py2rb_transpile.py`, confirming `checked=133 ok=133 fail=0 skipped=6`. Also added `ruby` target to `runtime_parity_check.py`, including `--targets ruby` path and Ruby entry verification in `test_runtime_parity_check_cli.py`.
