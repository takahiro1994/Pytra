...
Background:
- Nim previously had a record of passing sample parity, but the current `build_targets()` in `tools/check/runtime_parity_check.py` does not contain a `nim` target, so Nim has dropped out of continuous regression coverage.
...
- `tools/gen/regenerate_samples.py` also lacks Nim support, so the regeneration path for `sample/nim` is not fixed.
...
Scope:
- `tools/check/runtime_parity_check.py`, add the Nim target
- `tools/gen/regenerate_samples.py`, add Nim support
...
- `tools/unittest/test_runtime_parity_check_cli.py` and Nim-related smoke tests
