<a href="../../ja/plans/plan-pipeline-redesign.md"><img alt="Read in Japanese" src="https://img.shields.io/badge/docs-日本語-DC2626?style=flat-square"></a>
...
```
pytra-cli -build --target=cpp a.py   （parse→resolve→compile→optimize→link→emit を一気通貫）
```
...
```
pytra-cli -optimize *.east3  →  *.east3
```
...
CLI（`pytra-cli`）はファイル経由で各段を実行するが、テストスクリプトや parity check では API を直接呼んでファイル I/O をスキップできる。132 fixture × disk I/O で数分かかる処理がインメモリで大幅に高速化される。
...
- テストコード（`test/`）、ツール（`tools/`）、CLI のエントリポイント（`pytra-cli.py`）は selfhost 非対象。
- これらでは `Any`, Python 標準モジュール, `ast`, 動的 import の使用を許可する。
- **`toolchain2/` 配下のみが selfhost 対象。** `toolchain2/` 以外のファイルから `toolchain2/` の内部モジュールを import するのは自由だが、その逆（`toolchain2/` から `pytra-cli.py` や `toolchain/` を import すること）は禁止。

判定基準:
| ファイル | selfhost 対象 | `Any` | 動的 import | Python 標準 |
|---|---|---|---|---|
| `src/toolchain2/**/*.py` | **対象** | 禁止 | 禁止 | 禁止 |
| `src/pytra-cli.py` | **対象** | 禁止 | 禁止 | 禁止（`pytra.std.*` を使用） |
...
golden file の生成は `pytra-cli -golden` コマンドに一元化する。各 agent が独自スクリプトを作ることを禁止する。

```bash
# 各段の golden file 一括生成（現行 toolchain/ を使用）
pytra-cli -golden --stage=east1 --from=python -o test/east1/py/
pytra-cli -golden --stage=east2 --from=python -o test/east2/py/
pytra-cli -golden --stage=east3 -o test/east3/
pytra-cli -golden --stage=east3-opt -o test/east3-opt/
pytra-cli -golden --stage=emit --target=cpp -o test/emit/cpp/
```

- golden file 生成は `tools/gen/generate_golden.py`（selfhost 非対象）に分離する。`pytra-cli.py` の `-golden` サブコマンドは廃止予定。
- `pytra-cli.py` は selfhost 対象とし、`toolchain/` への依存を持たない。
...
PYTHONPATH=src python3 src/pytra-cli.py -parse sample/py/01_mandelbrot.py \
...
- `pytra-cli -golden --stage={east1,east2,east3,east3-opt} -o DIR` を実装。
...
- `pytra-cli -resolve` サブコマンドを実装し、CLI 経由での resolve を可能にした。
...
### 2026-03-25: [ID: P0-PARSE-S3] pytra-cli -parse を toolchain2 パーサーに切り替え

- `pytra-cli -parse` が toolchain/ ではなく toolchain2/parse/py/ を使うように切り替え。
- pytra.std.re のパターン追加（dotted/subscript/tuple assigns）。
- shim 版 re のバグ（二重 subscript 非対応）を発見。pytra.std.re は正しく動作。
- golden を pytra-cli 出力で再生成。fixture 132 + sample 18 = 150 件全件一致。
...
- `toolchain2/` は削除
- `pytra-cli.py` → `pytra-cli.py` にリネーム（旧 `pytra-cli.py` は `pytra-cli-legacy.py` に退避）
