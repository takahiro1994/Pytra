# Plan: Decouple C++-specific imports from pytra-cli.py (P0-CLI2-CPP-DECOUPLE)

## Background

Because `pytra-cli.py` imports `toolchain2.emit.cpp.runtime_bundle` at the top level, all language emitter modules end up in the dependency graph for Rust/Go selfhost builds, causing `cargo build` / `go build` to fail on compilation errors from modules that are not needed.

`pytra-cli.py` is meant to be a language-agnostic pipeline entry point, so having C++-specific imports at the top level is a design violation.

## Current State

```python
# top-level imports in pytra-cli.py
...
Move `_emit_cpp_linked_module()` to the `toolchain2.emit.cpp` side and call it from `pytra-cli.py` via a language-agnostic API.

### Specific Changes

1. Add `emit_cpp_linked_module(module, output_dir)` to `toolchain2.emit.cpp.cli` (or `toolchain2.emit.cpp.emit_linked`)
2. Remove `_emit_cpp_linked_module()` and the 3 `from toolchain2.emit.cpp.runtime_bundle import ...` lines from `pytra-cli.py`
3. Call through a language dispatch table in the `-emit` / `-build` C++ path of `pytra-cli.py`

### Language Dispatch Shape

```python
# pytra-cli.py — language-agnostic
...
- `pytra-cli.py` invokes each language's emitter as a subprocess for `-emit` / `-build`
...
- Fewer imports in `pytra-cli.py` (C++ dependency removed)
...
- `src/pytra-cli.py` no longer imports `toolchain2.emit.cpp.runtime_bundle`; C++ emit is now unified under a `python3 -m toolchain2.emit.cpp.cli` subprocess
- The `-build` C++ path also goes through the same subprocess emit after writing the link result
- Added and updated `tools/unittest/tooling/test_pytra_cli2.py` as a regression check; confirmed `pytra-cli.py -build ... --target cpp` succeeds for sample/fixture representatives
