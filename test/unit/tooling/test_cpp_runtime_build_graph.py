...
class CppRuntimeBuildGraphTest(unittest.TestCase):
    def _make_direct_header_project(self, workdir: Path) -> Path:
        ...
        (include_dir / "math.h").write_text(
            '\n'.join(
                [
                    "#pragma once",
                    '#include "runtime/generated/std/math.h"',
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_runtime_cpp_candidates_support_generated_native_layout_for_header_only_math(self) -> None:
        header = ROOT / "src/runtime/generated/std/math.h"
        ...
        self.assertNotIn((ROOT / "src/runtime/generated/std/math.cpp").as_posix(), paths)
    ...
    def test_runtime_cpp_candidates_from_generated_std_header_do_not_reintroduce_legacy_module_paths(self) -> None:
        header = ROOT / "src/runtime/generated/std/time.h"
        paths = [path.as_posix() for path in runtime_cpp_candidates_from_header(header)]
    ...
    def test_runtime_cpp_candidates_support_compiler_bucket(self) -> None:
        header = ROOT / "src/runtime/generated/compiler/backend_registry_static.h"
        paths = [path.as_posix() for path in runtime_cpp_candidates_from_header(header)]
        self.assertIn(
    ...
    def test_generated_compiler_header_prefers_native_cpp_only(self) -> None:
        header = ROOT / "src/runtime/generated/compiler/transpile_cli.h"
        paths = [path.as_posix() for path in runtime_cpp_candidates_from_header(header)]
        self.assertIn(
        ...
        self.assertNotIn(
            (ROOT / "src/runtime/generated/compiler/transpile_cli.cpp").as_posix(),
            paths,
        )
    ...
    def test_collect_runtime_sources_from_real_json_module_follows_direct_built_in_headers(self) -> None:
        module_sources = [str(ROOT / "src/runtime/generated/std/json.cpp")]
        runtime_sources = collect_runtime_cpp_sources(module_sources, ROOT / "src")
        self.assertIn("src/runtime/generated/std/json.cpp", runtime_sources)
        self.assertIn("src/runtime/generated/built_in/sequence.cpp", runtime_sources)
        self.assertIn("src/runtime/generated/built_in/string_ops.cpp", runtime_sources)
    ...
    def test_collect_runtime_sources_from_native_core_header_no_longer_pulls_removed_built_in_companions(
        ...
        self.assertIn("src/runtime/generated/built_in/string_ops.cpp", runtime_sources)
        self.assertNotIn("src/runtime/generated/built_in/numeric_ops.cpp", runtime_sources)
        self.assertNotIn("src/runtime/generated/built_in/zip_ops.cpp", runtime_sources)
        self.assertNotIn("src/runtime/generated/built_in/predicates.cpp", runtime_sources)
        self.assertNotIn("src/runtime/generated/built_in/sequence.cpp", runtime_sources)
        self.assertNotIn("src/runtime/generated/built_in/iter_ops.cpp", runtime_sources)
