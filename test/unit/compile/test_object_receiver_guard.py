...
class ObjectReceiverGuardTest(unittest.TestCase):
    ...
    def test_unknown_typed_receiver_is_rejected(self) -> None:
        """dict[str, Any].get() returns unknown → val.method() is rejected."""
        self._expect_rejection("test/fixtures/signature/ng_unknown_receiver.py")

    def test_dict_str_any_items_is_rejected(self) -> None:
        """dict[str, Any].get().items() is rejected (Any value method access)."""
        self._expect_rejection("test/fixtures/collections/dict_get_items.py")
