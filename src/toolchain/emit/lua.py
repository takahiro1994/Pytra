#!/usr/bin/env python3
"""Lua backend: link-output.json → Lua multi-file output.

Usage:
    python3 -m toolchain.emit.lua LINK_OUTPUT.json --output-dir out/lua/
"""
...
def main() -> int:
    ...
    if len(argv) == 0 or argv[0] in ("-h", "--help"):
        print("usage: toolchain.emit.lua LINK_OUTPUT.json --output-dir DIR")
        return 0
    ...
    for mod in modules:
        ...
        rel_path = module_id.replace(".", "/") + ".lua"
        out_path = out / rel_path
        out_path.parent.mkdir(parents=True, exist_ok=True)

        source = transpile_to_lua_native(east_doc, is_submodule=not is_entry)
