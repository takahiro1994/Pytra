#!/usr/bin/env python3
"""Standalone PHP backend: EAST3 JSON → PHP source.
...
def main() -> int:
    ...
    if len(argv) == 0 or argv[0] in ("-h", "--help"):
        print("usage: toolchain.emit.php INPUT.json -o OUTPUT.php")
        return 0
    ...
    if input_path == "":
        ...
        return 1
    if output_path == "":
        output_path = Path(input_path).stem + ".php"

    east_doc = json.loads(Path(input_path).read_text(encoding="utf-8"))
    source = transpile_to_php_native(east_doc)
