#!/usr/bin/env python3
"""Standalone Swift backend: EAST3 JSON → Swift source.
...
def main() -> int:
    ...
    if len(argv) == 0 or argv[0] in ("-h", "--help"):
        print("usage: toolchain.emit.swift INPUT.json -o OUTPUT.swift")
        return 0
    ...
    if input_path == "":
        ...
        return 1
    if output_path == "":
        output_path = Path(input_path).stem + ".swift"

    east_doc = json.loads(Path(input_path).read_text(encoding="utf-8"))
    source = transpile_to_swift(east_doc)
