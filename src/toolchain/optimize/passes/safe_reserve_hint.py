...
from toolchain.common.nodes import const_int_node, const_int_value, binop_expr, compare_expr, ifexp_expr


_CONTROL_FLOW_KINDS: set[str] = {"If", "While", "For", "ForCore", "Try", "With"}


def _collect_target_names(target_node: JsonVal, out: set[str]) -> None:
...
def _collect_target_plan_names(target_plan: JsonVal, out: set[str]) -> None:
...
def _collect_assigned_names(node: JsonVal, out: set[str]) -> None:
    ...
    elif kind == "For" or kind == "ForRange":
        _collect_target_names(node.get("target"), out)
    elif kind == "ForCore":
...
def _collect_name_refs(node: JsonVal, out: set[str]) -> None:
    if isinstance(node, list):
        for item in node:
...
def _clone_expr(node: JsonVal) -> dict[str, JsonVal] | None:
    if not isinstance(node, dict):
        return None
    cloned = deep_copy_json(node)
    return cloned if isinstance(cloned, dict) else None


def _is_const_int_expr(expr: JsonVal, expected: int) -> bool:
    return const_int_value(expr) == expected
...
def _build_static_range_trip_count_expr(
    *,
    ...
    zero_expr = const_int_node(0)
    if range_mode == "ascending":
        test = compare_expr("LtE", _clone_expr(stop) or stop, _clone_expr(start) or start)
        if step_abs == 1:
            if _is_const_int_expr(start, 0):
                positive = _clone_expr(stop)
            else:
                positive = binop_expr("Sub", _clone_expr(stop) or stop, _clone_expr(start) or start)
        else:
            diff = binop_expr("Sub", _clone_expr(stop) or stop, _clone_expr(start) or start)
            numer = binop_expr("Add", diff, const_int_node(step_abs - 1))
            positive = binop_expr("Div", numer, const_int_node(step_abs))
        if not isinstance(positive, dict):
            return None
        return ifexp_expr(test, zero_expr, positive)

    if range_mode == "descending":
        test = compare_expr("GtE", _clone_expr(stop) or stop, _clone_expr(start) or start)
        if step_abs == 1:
            if _is_const_int_expr(stop, 0):
                positive = _clone_expr(start)
            else:
                positive = binop_expr("Sub", _clone_expr(start) or start, _clone_expr(stop) or stop)
        else:
            diff = binop_expr("Sub", _clone_expr(start) or start, _clone_expr(stop) or stop)
            numer = binop_expr("Add", diff, const_int_node(step_abs - 1))
            positive = binop_expr("Div", numer, const_int_node(step_abs))
        if not isinstance(positive, dict):
            return None
...
def _is_simple_invariant_expr(expr: JsonVal, mutated_names: set[str]) -> bool:
    if not isinstance(expr, dict):
        return False
    kind = expr.get("kind")
    if kind == "Constant":
        return const_int_value(expr) is not None
    if kind == "Name":
        ident = expr.get("id")
...
def _top_level_unconditional_append_owner(body: list[JsonVal]) -> str:
    append_count = 0
    append_owner = ""
...
class SafeReserveHintPass(East3OptimizerPass):
    ...
    name: str = "SafeReserveHintPass"
    min_opt_level: int = 1

    def _set_hints(self, stmt: dict[str, JsonVal], hints: list[dict[str, JsonVal]]) -> int:
    ...
    def _try_tag_forcore(self, stmt: dict[str, JsonVal]) -> int:
        ...
        step_val = const_int_value(step_obj)
        ...
        if range_mode != "ascending" and range_mode != "descending":
            return self._set_hints(stmt, [])
