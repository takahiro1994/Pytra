...
def _contains_name(node: JsonVal, ident: str) -> bool:
    if isinstance(node, list):
        for item in node:
...
class UnusedLoopVarElisionPass(East3OptimizerPass):
    ...
    name: str = "UnusedLoopVarElisionPass"
    min_opt_level: int = 1

    def _try_elide_forcore(self, stmt: dict[str, JsonVal], later_stmts: list[JsonVal]) -> bool:
