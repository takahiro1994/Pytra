...
def _build_target_plan(
    ...
    *,
    ...
    if isinstance(target, dict):
        ...
        kind = td.get("kind")
        if kind == "Name":
            out: Node = {"kind": "NameTarget", "id": td.get("id", "")}
            if tt_norm != "unknown":
        ...
        if kind == "Tuple":
            ...
            if tt_norm != "unknown":
                out["target_type"] = tt_norm
            return out
    out = {"kind": "ExprTarget", "target": lower_node_fn(target)}
    if tt_norm != "unknown":
        out["target_type"] = tt_norm
    return out
...
def _decorate_nominal_adt_variant_pattern(pattern: Node) -> Node:
    ...
    for item in subs:
        if isinstance(item, dict) and item.get("kind") == "PatternBind":
    ...
    for idx in range(len(subs)):
        ...
        if spd.get("kind") == "PatternBind":
            ...
            pb_meta: Node = {"schema_version": 1, "ir_category": "NominalAdtPatternBind",
...
def _decorate_nominal_adt_match_stmt(match_stmt: Node) -> Node:
    ...
    for case in cases:
        ...
        for si in range(len(subs3l)):
            ...
            if bn != "":
                bnames.append(bn)
            spd2["lowered_kind"] = "NominalAdtPatternBind"
...
def _build_or_of_checks(checks: list[Node], source_expr: JsonVal) -> Node:
    if len(checks) == 1:
        return checks[0]
    out: Node = {
        "kind": "BoolOp", "op": "Or", "values": checks,
...
def _desugar_vararg_funcdef(nd: Node) -> Node:
    ...
    if isinstance(ate, dict):
        ...
        if isinstance(vte, dict):
            ated[vn] = {"kind": "GenericType", "base": "list", "args": [vte]}
        else:
            ated[vn] = {"kind": "GenericType", "base": "list", "args": [{"kind": "NamedType", "name": vt}]}
