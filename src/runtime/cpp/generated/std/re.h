...
struct Match : public RcObject {
    ...
    str group(int64 idx = 0) const {
        if (idx == 0)
            return this->_text;
        if ((idx < 0) || (idx > (this->_groups).size()))
...
::std::optional<rc<Match>> match(const str& pattern, const str& text, int64 flags = 0) {
    ...
    if (pattern == "^([A-Za-z_][A-Za-z0-9_]*)\\[(.*)\\]$") {
        ...
        if (!(_is_ident(head)))
            return ::std::nullopt;
        return ::rc_new<Match>(text, list<str>{head, py_str_slice(text, i + 1, -(1))});
    }
    ...
    if (pattern == "^(any|all)\\((.+)\\)$") {
        if ((py_startswith(text, "any(")) && (py_endswith(text, ")")) && (text.size() > 5))
            return ::rc_new<Match>(text, list<str>{"any", py_str_slice(text, 4, -(1))});
        if ((py_startswith(text, "all(")) && (py_endswith(text, ")")) && (text.size() > 5))
            return ::rc_new<Match>(text, list<str>{"all", py_str_slice(text, 4, -(1))});
        return ::std::nullopt;
    }
    ...
    if (pattern == "^type\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*(.+)$") {
        ...
        if ((!(_is_ident(name))) || (rhs == ""))
            return ::std::nullopt;
        return ::rc_new<Match>(text, list<str>{name, rhs});
    }
