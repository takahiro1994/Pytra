...
struct JsonArr {
    ...
    JsonArr(const rc<list<JsonVal>>& raw) {
        this->raw = raw;
    }
...
struct JsonValue {
    ...
    ::std::optional<JsonArr> as_arr() const {
        ...
        if ((jv).tag == PYTRA_TID_LIST)
            return JsonArr(py_to<rc<list<JsonVal>>>(jv.as<list>()));
        return ::std::nullopt;
    }
...
::std::optional<JsonArr> loads_arr(const str& text) {
    ...
    if ((val).tag == PYTRA_TID_LIST)
        return JsonArr(py_to<rc<list<JsonVal>>>(val.as<list>()));
    return ::std::nullopt;
}
...
str _dump_json_value(const JsonVal& v, bool ensure_ascii, const ::std::optional<int64>& indent, const str& item_sep, const str& key_sep, int64 level) {
    ...
    if ((v).tag == PYTRA_TID_LIST)
        return _dump_json_list(py_to<rc<list<JsonVal>>>(v.as<list>()), ensure_ascii, indent, item_sep, key_sep, level);
