...
`--category` でカテゴリ絞り込み可能。結果は `work/parity-results/<target>_<case-root>.json` に自動蓄積される。
...
parity 結果の蓄積も同様に `work/parity-results/` に行う。
...
`work/parity-results/<target>_<case-root>.json` に蓄積されたパリティ結果を読み込み、バックエンド進捗ページを生成する。
